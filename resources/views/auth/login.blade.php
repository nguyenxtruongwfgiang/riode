        ...
        <div class="flex items-center justify-center mt-4">
            <div class="block mt-4">
                @if (Route::has('register'))
                    <span class="text-sm text-gray-600">{{ __("Don't have account? ") }}</span><a class="underline text-sm text-gray-600 hover:text-blue-700 rounded-md focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500"
                        href="{{ route('register') }}">
                        {{ __('Register') }}
                    </a>
                @endif
